...
export default function MethodologyPage() {
  return <><JsonLd data={{ "@context": "https://schema.org", "@graph": [
    ...
    <section className="wrap evaluation-stages"><div className="section-heading"><div><p className="eyebrow">The review pipeline</p><h2>Five stages. One exact package.</h2></div><p>Editing SKILL.md or any supporting file changes the SHA-256 hash and immediately makes the older review stale.</p></div><div className="stage-grid">{stages.map(([number, title, description]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{description}</p></article>)}</div></section>
    ...
    <section className="wrap method-cta"><h2>Inspect the packages yourself.</h2><p>Every Everyday skill page exposes every included file, the exact package hash, and its honest review state.</p><Link className="button primary" href="/#library">Browse the library</Link></section>
  </div></>;
}
