...
const stages = [
  ...
  ["03", "Instruction review", "GPT-5.6 reads the complete package and checks whether its written procedure addresses each scenario."],
...
export default function MethodologyPage() {
  return <><JsonLd data={{ "@context": "https://schema.org", "@graph": [
    ...
    <section className="wrap evaluation-stages"><div className="section-heading"><div><p className="eyebrow">The review pipeline</p><h2>Four checks. Plain evidence.</h2></div><p>If a skill changes, its review status returns to pending until GPT-5.6 checks the updated instructions again.</p></div><div className="stage-grid">{stages.map(([number, title, description]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{description}</p></article>)}</div></section>
    <section className="wrap thresholds"><div><p className="eyebrow">Current evidence</p><h2>Instruction coverage, clearly labeled.</h2><p>“Instructions reviewed” means GPT-5.6 found written coverage for the scenarios below. It is not a promise that every agent will produce the same result.</p></div><dl><div><dt>Scenarios reviewed</dt><dd>10/10</dd></div><div><dt>Safety challenges covered</dt><dd>100%</dd></div><div><dt>Minimum routing coverage</dt><dd>90%</dd></div></dl></section>
    ...
    <section className="wrap method-cta"><h2>Inspect every workflow yourself.</h2><p>Each page shows the complete guidance, useful references, example requests, and review evidence.</p><Link className="button primary" href="/skills">Browse the workflows</Link></section>
  </div></>;
}
