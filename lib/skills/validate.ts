...
export function validateSkill(
  ...
  if (/\b(TODO|TBD|PLACEHOLDER)\b/i.test(markdown))
    errors.push("SKILL.md contains unfinished placeholder text.");
  if (
    /think like|in the style of|act as (?:a |an )?(?:celebrity|guru)/i.test(
      parsed.content,
    )
  )
    errors.push("Skill contains persona imitation language.");
  ...
  for (const match of parsed.content.matchAll(
    ...
    if (!target.includes(":") && !paths.has(target))
      errors.push(`Broken package link: ${match[1]}.`);
  }
  const metadata = files.find((file) => file.path === "agents/openai.yaml");
  if (!metadata) errors.push("Missing agents/openai.yaml.");
  else {
    const short =
      metadata.content.match(/short_description:\s*"([^"]+)"/)?.[1] ?? "";
    const prompt =
      metadata.content.match(/default_prompt:\s*"([^"]+)"/)?.[1] ?? "";
    if (short.length < 25 || short.length > 64)
      errors.push("Agent short_description must be 25–64 characters.");
    if (!prompt.includes(`$${catalog.slug}`))
      errors.push("Agent default_prompt must mention the skill by $name.");
  }
  return {
