...
describe("library memory", () => {
  it("saves and restores a validated state", () => {
    ...
    const state = LibraryMemory.recordRecent(
      LibraryMemory.toggleSaved(LibraryMemory.empty(), "plan-your-week"),
      "plan-your-week",
    );
    expect(LibraryMemory.write(storage, state)).toEqual({
      ok: true,
      value: state,
    });
  ...
  it("rejects malformed browser data", () => {
    const result = LibraryMemory.read(
      memoryStorage('{"saved":[42],"recent":[]}'),
    );
  ...
  it("keeps recent workflows unique and bounded", () => {
    ...
    const state = slugs.reduce(
      (memory, slug) => LibraryMemory.recordRecent(memory, slug),
      LibraryMemory.empty(),
    );
