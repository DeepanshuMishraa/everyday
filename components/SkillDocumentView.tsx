...
export function SkillDocumentView({ body, files }: { body: string; files: SkillPackageFile[] }) {
  ...
  return <section className="document-shell" aria-labelledby="document-heading">
    <div className="document-toolbar"><div><p className="eyebrow">Skill package — {files.length} files</p><h2 id="document-heading">{file.path}</h2></div><div className="segmented" role="group" aria-label="Document view"><button className={view === "rendered" ? "active" : ""} onClick={() => setView("rendered")}>Read</button><button className={view === "raw" ? "active" : ""} onClick={() => setView("raw")}>Raw</button></div></div>
