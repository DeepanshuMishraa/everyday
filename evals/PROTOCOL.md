...
The report must contain `reviewer: "codex"`, the exact current package hash, and ten scenario review records. Editing any file makes the report stale. `npm run check-evals` verifies these conditions without performing or pretending to perform the semantic judgment.
